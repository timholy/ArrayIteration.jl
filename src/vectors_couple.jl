...
function copy!(dest, src)
    for (idest, isrc) in couple(any, dest, src)  # any should be the default
        dest[idest] = src[isrc]
    end
    dest
end
...
function vecdot(x::AbstractVector, y::AbstractVector)
    ...
    citer = couple(all, x, y)  # all, because a*b = 0 unless both are nonzero
    cstate = start(citer)
